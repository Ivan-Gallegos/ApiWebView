...
class Repository private constructor(
    ...
    suspend fun getQueryResponse(query: String): String = withContext(Dispatchers.IO) {
     ...
     */
    private suspend fun getValidDbResponse(query: String): String? = withContext(Dispatchers.IO) {
        db.searchDao().getSearchEntity(query).let { entity ->
            if (entity != null && entity.ageMillis <= MAX_AGE_MILLIS) entity.response
            else null
        }
    }
     ...
     */
    private suspend fun getNetworkResponse(query: String) = withContext(Dispatchers.IO) {
        try {
            val response = pokeService.getPokemon(query)
            if (response.isSuccessful) (response.body() ?: "Empty body")
                .also { db.searchDao().insertWithTimeStamp(query, it) }
            else response.errorBody()?.string() ?: "Empty Error Message"
        } catch (e: Exception) {
